...
\thanks{V. Panghal is with the Department of Artificial Intelligence and Data Science, Poornima Institute of Engineering and Technology (PIET), Jaipur, India. Correspondence: \texttt{pvt.panghal@gmail.com}.}%
...
In this work, we demonstrate that these two phenomena are deeply entangled, and that the prevailing belief in unavoidable temporal calibration decay is in fact a misattribution. When models are trained with positive loss weighting $w > 1$, the asymptotic Bayes-optimal logit is shifted by $+\log w$~\cite{king2001logistic, caplin2022calibrating}. In clinical risk prediction, Piccininni et al.~\cite{piccininni2024understanding} similarly showed that imbalance correction through resampling distorts probability targets and damages calibration, while plug-in corrections can recover valid estimates. In theoretical machine learning, Caplin et al.~\cite{caplin2022calibrating} showed that class weighting is fundamentally incompatible with raw probability calibration and proposed an inverse formulation to recover true likelihoods, validated on a static pneumonia chest X-ray task. Standard post-hoc calibration methods—most notably temperature scaling~\cite{guo2017calibration}—only rescale the logit variance ($z / T$) and cannot shift log-odds. Consequently, weighted models continuously over-predict positive probabilities, producing severe Brier score and Expected Calibration Error (ECE) degradation that worsens over time as disease prevalence fluctuates.
...
\begin{enumerate}
    ...
    \item \textbf{Operational Selective Prediction and Workload Triage Analysis:} Extending multi-dataset selective CXR frameworks~\cite{aperstein2026knowing}, we utilize selective prediction as an operational downstream lens with explicit tracking of radiologist referral workloads, retained sensitivity, and automated false negatives under matched development thresholds, establishing practical deployment trade-offs under temporal shift.
\end{enumerate}
...
\end{equation}
Equation~(\ref{eq:bayes_logit}) proves that positive loss weighting shifts the model's output logits by $+\log w$, consistent with classical choice-based sampling adjustments~\cite{king2001logistic} and the class-weight likelihood recovery formulation of Caplin et al.~\cite{caplin2022calibrating}. When $w \gg 1$, the network is mathematically incentivized to output large positive logits, systematically inflating predicted probabilities.
...
\begin{table*}[!t]
\caption{Experimental Matrix Evaluation: Longitudinal Trajectories across Architectures, Loss Objectives, and Thoracic Pathologies}
